  ...
  hooks: {
    beforeValidate: [
      ({ req }) => {
        const file = req.file
        if (file) {
          // SVGs are disabled by default because they can contain malicious code (XSS)
          // that can be executed in the browser. If you need to allow SVGs,
          // make sure you have a proper sanitization process in place.
