...
export const revalidateService: CollectionAfterChangeHook = ({
  ...
  if (doc._status === 'published') {
    ...
    payload.logger.info(`Service page published at path: ${path}`)
  ...
  if (previousDoc?._status === 'published' && doc._status !== 'published') {
    ...
    payload.logger.info(`Service page unpublished at path: ${oldPath}`)
  }
  ...
  if (previousDoc?.slug && doc.slug !== previousDoc.slug && previousDoc._status === 'published') {
    const oldPath = `/services/${previousDoc.slug}`
    const newPath = `/services/${doc.slug}`

    payload.logger.info(`Service page slug changed from ${oldPath} to ${newPath}`)
  }
