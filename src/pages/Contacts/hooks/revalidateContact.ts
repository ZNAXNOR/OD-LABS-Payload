...
export const revalidateContact: CollectionAfterChangeHook = ({
  ...
  if (doc._status === 'published') {
    ...
    const path = `/contact/${doc.slug}`

    payload.logger.info(`Contact page published at path: ${path}`)
  ...
  if (previousDoc?._status === 'published' && doc._status !== 'published') {
    const oldPath = `/contact/${previousDoc.slug}`

    payload.logger.info(`Contact page unpublished at path: ${oldPath}`)
  }
  ...
  if (previousDoc?.slug && doc.slug !== previousDoc.slug && previousDoc._status === 'published') {
    const oldPath = `/contact/${previousDoc.slug}`
    const newPath = `/contact/${doc.slug}`

    payload.logger.info(`Contact page slug changed from ${oldPath} to ${newPath}`)
  }
