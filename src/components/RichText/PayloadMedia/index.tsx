import { type Media, type PayloadMediaBlock as PayloadMediaBlockType } from '@/payload-types'
...
export const PayloadMediaBlock: React.FC<PayloadMediaBlockType> = (fields) => {
  const { data: media } = usePopulateDocument<Media>({
    id: typeof fields.media === 'object' ? String(fields.media?.id) : String(fields.media),
    collection: 'media',
    depth: 1,
  ...
  return (
    <LightDarkImage
      ...
      srcDarkId={
        typeof media?.darkModeFallback !== 'object' && media?.darkModeFallback
          ? String(media?.darkModeFallback)
          : undefined
      }
